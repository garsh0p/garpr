...
tournament_import_parser.add_argument('smashgg_url', type=str, required=True)
...
class PlayerListResource(restful.Resource):
    ...
    def get(self, region):
        ...
        # search multiple players by name across all regions
            ...
            return_dict['players'] = [p.get_json_dict() for p in
                                      self._get_players_matching_query(all_players, args['query'])]
...
class TournamentResource(restful.Resource):
    def get(self, region, id):
        ...
        if tournament is not None:
            ...
            user = get_user_from_request(request, dao)
            pending_tournament = dao.get_pending_tournament_by_id(
                ObjectId(id))  # this usage is safe, if the ID was fake, we would have already blown the coop above
    ...
    def put(self, region, id):
        ...
        try:
            ...
            if args['matches']:
                ...
                matches = [MatchResult(winner=ObjectId(m['winner']), loser=ObjectId(m['loser'])) for m in
                           args['matches']]
...
def add_cors(resp):
    ...
    resp.headers['Access-Control-Allow-Origin'] = request.headers.get('Origin',
                                                                      '*')  # !!! this needs to be tightened down to only the domains we're expecting
